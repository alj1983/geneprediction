...
sub local_database_search {
    if ( $_[0] eq '' ) {
	die "fasta file with protein sequence(s) has to be provided as argument to the script\n"; }
...
sub best_hits_to_fasta {
    if ($_[0] eq '' ) { die "Compinfo has to be provided as input\n"; }
...
# A stop codon is shown as '*'
    ...
    if ($_[1] eq '') { die "The file Compinfo is needed as second input file"};
